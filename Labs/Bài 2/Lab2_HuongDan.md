...
→ **Giải pháp:** Viết automation script!

### 2.2 Tạo Automation Script

Tạo thư mục:

```bash
mkdir devops-lab2-automation
cd devops-lab2-automation
```

Tạo file `setup-dev-env.sh` (macOS/Linux):
...
echo "Next: cd $(pwd) && code ."
```

Tạo file `setup-dev-env.ps1` (Windows PowerShell):
...
**Kết quả mong đợi:** Tự động tạo cấu trúc dự án + .gitignore + README trong < 5 giây!

### 2.4 Phân tích — Nguyên tắc Automation

| Trước (Thủ công) | Sau (Automation Script) |
|-------------------|------------------------|
| Tạo thư mục bằng tay | Script tự tạo |
| Tạo .gitignore bằng tay | Script tự sinh |
| Cài đặt từng bước → 10 phút | 1 lệnh → 5 giây |
